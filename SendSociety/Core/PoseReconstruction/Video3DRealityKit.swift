...
enum Video3DRealityKit {
    ...
    static func centerDepthMeters(from context: AppleVisionSkeletonExtractor.DepthGroundingContext) -> Float? {
    ...
    static func generate3DJointPositions(
        ...
        guard let depthContext else {
            ...
            for (joint, local) in sample.rootRelativePositions {
                worldPositions[joint] = AppleVisionSkeletonExtractor.worldPosition(
                    rootRelative: local,
        ...
        for (joint, cameraSpace) in grounded {
            worldPositions[joint] = AppleVisionSkeletonExtractor.worldPosition(cameraSpace: cameraSpace, cameraTransform: cameraTransform)
        }
        return keepInFrontOfWall(worldPositions, wallReference: wallReference)
    }
...
//        }
    ...
    static func skeletonEntity(
        ...
        let worldPositions = overridePositions ?? sample.map {
            generate3DJointPositions(from: $0, cameraTransform: cameraTransform, depthContext: depthContext, wallReference: wallReference)
        } ?? [:]
