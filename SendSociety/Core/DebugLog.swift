...
/// Structured logging aligned to the 5 MVP success criteria in the build brief. Each criterion
...
/// 2. calibration      — are limb lengths stable across repeated attempts on the same person?
/// 3. recording         — does depth stay correctly associated with each video frame?
/// 4. reconstruction   — does the skeleton line up with the wall in Step 4?
/// 5. tracking          — does tracking survive handheld use, or is a tripod required?
enum DebugLog {
    ...
    static let calibration = Logger(subsystem: subsystem, category: "2-calibration-stability")
