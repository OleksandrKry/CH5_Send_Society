...
struct ARFrameData {
    ...
    var depthGroundingContext: AppleVisionSkeletonExtractor.DepthGroundingContext? {
        guard let depthMap else { return nil }
        return AppleVisionSkeletonExtractor.DepthGroundingContext(
...
final class ARFrameStore {
    private(set) var framesByTimestamp: [TimeInterval: ARFrameData] = [:]
    ...
    /// (success criterion #3).
    func nearestFrame(toPlaybackSeconds playbackSeconds: TimeInterval, clipStartTimestamp: TimeInterval) -> ARFrameData? {
        guard let start = recordingStartTimestamp, !sortedTimestamps.isEmpty else { return nil }
        let target = clipStartTimestamp + playbackSeconds
